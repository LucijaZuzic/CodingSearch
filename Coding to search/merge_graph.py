...
yes_words_lists = [
    ...
    ["behavior", "behaviour"], 
    ["watercraft"],
    ["smuggling", "smuggle", "illegal traffic"],
    ["piracy", "pirate"],
    ["traffic"], 
    ["illegal entry"],
    ...
    ["anomalous trajectories", "AT"],
    ["graph"],
    ["on-off switching", "on off switching", "OOS"],
    ["variational", "VRNN"],
    ["encode", "decode", "autoencoder"],
    ["deep learning", "DL"],
...
for yes_words in yes_words_lists:
    ...
    for paper in aggregate_cols:  
        ...
        for any_of in any_words_list:
            found_any = False
            for x in any_of:
                if aggregate_cols[paper].count(x):
                    found_any = True
                    break
            if not found_any:
                banned = True
